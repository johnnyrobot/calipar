...
- [ ] **Step 6:** Against that exact URL: `npm run verify:headers -- <url>` — ~~currently unreachable from any npm script, wire it up~~ **already wired** at `package.json:42`, this parenthetical is stale. Then routes, manifest, service worker, offline behaviour, Cache Storage, and API error shapes — **now one command: `npm run verify:preview -- <url>`** (`scripts/verify/preview.mjs`, added 2026-08-04, self-tested at 45 passed / 1 failed against a local `wrangler dev`, the single failure being the deliberate secrets-missing detector). It needs no secrets and no session, so it is safe to point at production too. Note `verify:headers` only asserts `cache-control` on `/sw.js`, which is why it did not catch the `_headers` append defect fixed in `f680b37`; `verify:preview` carries an explicit guard for that. **Cache Storage and offline-after-install remain out of scope for both** — they need a real service worker and stay in `tests/e2e/pwa.spec.ts` (Chromium only).
