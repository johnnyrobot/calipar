...
- [ ] **Step 1:** Add `noindex` for the unlisted beta — a `public/robots.txt` with `Disallow: /` plus `<meta name="robots" content="noindex">` in the root layout. Add `robots.txt` to both artifact `required` arrays. Remove both at GA.
- [ ] **Step 2:** `uptime`, then a clean `npm ci && npm run verify:full` under **Node 20** (CI's version; all local evidence to date is Node 22).
- [ ] **Step 3:** Confirm the Cloudflare account, Workers Free plan, and Worker-name ownership with authenticated Wrangler. Do not infer any of it.
...
- [ ] **Step 6:** Against that exact URL: `node scripts/verify/headers.mjs <url>` (currently unreachable from any npm script — wire it up as `verify:headers`), then routes, manifest, service worker, offline behaviour, Cache Storage, and API error shapes.
...
| D | `npm run verify:full` on Node 20 from a clean `npm ci` | All gates pass |
