...
export default function ResourcesPage() {
  ...
  if (!data) return null;
  const review = data.reviews.find((item) => item.status === "draft") ?? data.reviews[0];
  const total = data.resourceRequests.reduce((sum, request) => sum + request.amountCents, 0);
  ...
  return (
    <div>
      ...
      <section className="resource-summary">
        <div><span>REQUESTED</span><strong>{currency(total)}</strong><p>{data.resourceRequests.length} browser-local requests</p></div>
        <div className="resource-distribution">
          {(["requested", "recommended", "funded", "declined"] as const).map((status) => <span key={status}><i className={status} />{status}<strong>{data.resourceRequests.filter((item) => item.status === status).length}</strong></span>)}
        </div>
      </section>
      <section className="panel request-table">
        <div className="panel-head"><div><p className="eyebrow">PRIORITIZED PIPELINE</p><h2>Resource requests</h2></div></div>
        <div className="table-scroll">
          <table>
            ...
            <tbody>{[...data.resourceRequests].sort((a,b) => a.priority-b.priority).map((request) => <tr key={request.id}><td><span className="priority-number">{request.priority}</span></td><td><strong>{request.title}</strong><small>{request.rationale}</small></td><td>{request.objectCodeSeries} series</td><td>{currency(request.amountCents)}</td><td><span className={`status-pill ${request.status}`}>{request.status}</span></td><td><button aria-label={`Delete ${request.title}`} className="icon-button resource-delete-button" type="button" onClick={() => setDeleteItem(request)}><Icon name="close" /></button></td></tr>)}</tbody>
          </table>
        </div>
      </section>
